...
require 'bit'
base64 = SMODS.load_file("lib/base64.lua")()

---@class Penumbra.MusicTag
...
PBJukebox = {}
---@type Penumbra.MusicTag[]
PBJukebox.MusicTags = {}
---@type table<string, Penumbra.MusicTag>
PBJukebox.Music = {}
PBJukebox.Current = 0
...
function decode_vorbis_comment_img(str, name)
	...
	--NFS.write(Penumbra.path .. name .. "_og.jpg", decoded)
	...
	-- actual image data
	NFS.write(name .. ".jpg", string.sub(decoded, idx, idx + len - 1))
	local byte_data = NFS.newFileData(name .. ".jpg")
...
---@return Penumbra.MusicTag
function PBJukebox.read_music_tags(path, music_name)
	...
	---@type Penumbra.MusicTag
	local info = { key = music_name, order = #PBJukebox.MusicTags + 1, mod = SMODS.Sounds[music_name].mod, pbra_credits = SMODS.Sounds[music_name].pbra_credits }
	-- POST: Check if we defined this in the SMODS.Sound definition instead
	if SMODS.Sounds[info.key] then
		info.title = SMODS.Sounds[info.key].pbra_title or nil
		info.artist = SMODS.Sounds[info.key].pbra_artist or nil
	end
	if idx > 0 then
		...
		for i = 1, comment_size do
			...
			for key, value in string.gmatch(comment, '([^=]+)=(.*)') do
				...
				if comment_name == "metadata_block_picture" then
					...
					if success then
						G.ASSET_ATLAS[(music_name or path)] = G.ASSET_ATLAS[(music_name or path)] or atlas
					else
...
SMODS.Sound.inject = function(self, i)
	old_inject(self, i)
	if string.find(self.key, "music_") then
		local _, tag = pcall(PBJukebox.read_music_tags, self.full_path, self.key)
		-- check if it should be discoverable
		if self.pbra_discoverable then
			tag.discoverable = true
		end
		PBJukebox.Music[tag.key] = tag
        PBJukebox.MusicTags[#PBJukebox.MusicTags + 1] = tag
	end
end
...
function set_profile_progress()
	...
	G.PROFILES[G.SETTINGS.profile]["pbra_discovered_tracks"] = G.PROFILES[G.SETTINGS.profile]["pbra_discovered_tracks"] or
		{}
	for tag, t in pairs(PBJukebox.Music) do
		if G.PROFILES[G.SETTINGS.profile]["pbra_discovered_tracks"][tag] and t.discoverable then
			t.discovered = true
		end
	end
end
...
function G.FUNCS.unlock_all(...)
    ...
    for tag, t in pairs(PBJukebox.Music) do
		if t.discoverable then
            G.PROFILES[G.SETTINGS.profile]["pbra_discovered_tracks"][tag] = true
			t.discovered = true
		end
	end
end
...
PBJukebox.create_collection_ui = function(_pool, rows, args)
	args = args or {}
...
local function jukebox_collection_ui(e)
	return PBJukebox.create_collection_ui(PBJukebox.MusicTags, { 6, 6, 6 }, {
		snap_back = true,
		...
		modify_card = function(card, center)
			local atlas = (center.discoverable and not center.discovered) and G.ASSET_ATLAS["pbra_jukebox_undiscovered"] or
				G.ASSET_ATLAS[center.key] or G.ASSET_ATLAS["pbra_jukebox_default"]
			local scalex = 159 / atlas.px
			local scaley = 159 / atlas.py
			local temp_blind = Sprite(card.children.center.T.x, card.children.center.T.y, 2.2*scalex, 2.2*scaley,
				atlas, { x = 0, y = 0 })
			...
			if center.discoverable and not center.discovered then
				...
				spr = Sprite(card.children.center.T.x, card.children.center.T.y, 2.2, 2.2,
					G.ASSET_ATLAS["pbra_jukebox_undiscovered"], { x = 1, y = 0 })
			...
			card.pbra_jukebox_key = center.key
			local old_click = card.click
			card.click = function(self)
				old_click(self)
				if not (center.discoverable and not center.discovered) then
					PBJukebox.CurrentlyPlaying = Penumbra.get_name(center.key, Penumbra.tags[center.key] or {}) .. " - " .. Penumbra.get_artist(center.key, Penumbra.tags[center.key] or {})
					PBJukebox.ActuallyPlaying = center.key
					PBJukebox.Current = PBJukebox.Music[center.key].order
...
G.FUNCS.your_collection_pbra_jukebox = function()
...
function G.FUNCS.pbra_jukebox_play(e)
	G.FUNCS.your_collection_pbra_jukebox(e)
end

function G.FUNCS.pbra_jukebox_shuffle(e)
	local pool = {}
	for k, v in pairs(PBJukebox.MusicTags) do
		if not (v.discoverable and not v.discovered) then
			pool[#pool+1] = v
		end
	end
	local random = pseudorandom_element(pool, "pbra_jukebox_shuffle")
	if random then
		PBJukebox.CurrentlyPlaying = Penumbra.get_name(random.key, Penumbra.tags[random.key] or {}) .. " - " .. Penumbra.get_artist(random.key, Penumbra.tags[random.key] or {})
		PBJukebox.ActuallyPlaying = random.key
		PBJukebox.Current = PBJukebox.Music[random.key].order
		G.FUNCS[G.ACTIVE_MOD_UI and "openModUI_" .. G.ACTIVE_MOD_UI.id or 'your_collection'](e)
	end
end

function G.FUNCS.pbra_jukebox_stop(e)
	PBJukebox.CurrentlyPlaying = localize('pbra_jukebox_default_music_title')
	PBJukebox.ActuallyPlaying = nil
	PBJukebox.Current = 0
	G.ALBUM_CARD:start_dissolve()
	G.ALBUM_CARD.pbra_jukebox_none = true
	...
	local uibox = e.UIBox:get_UIE_by_ID("pbra_jukebox_desc")
	if uibox then
		G.ALBUM_DESC = UIBox {
			definition = desc,
...
function G.FUNCS.pbra_jukebox_can_stop_playback(e)
	if PBJukebox.ActuallyPlaying then
		e.config.colour = G.C.RED
		e.config.button = 'pbra_jukebox_stop'
	else
...
-- Jukebox tab lmao
function PBJukebox.MusicTab()
	for i, v in pairs(Penumbra.loc_keys) do
		--this is actually so gross
		G.localization.descriptions.pbra_jukebox[i].name = G.localization.descriptions.pbra_jukebox[i].name or
		{ Penumbra.get_name(i, Penumbra.tags[i] or {}), "{C:edition,s:0.6}" .. Penumbra.get_artist(i, Penumbra.tags[i] or {}) }
		G.localization.descriptions.pbra_jukebox[i].name_parsed = nil
		G.localization.descriptions.pbra_jukebox[i].text_parsed = nil
		Penumbra.manual_parse(G.localization.descriptions.pbra_jukebox[i], Penumbra.tags[i] or {})
	end
	local current = PBJukebox.MusicTags[PBJukebox.Current]
	PBJukebox.CurrentlyPlaying = PBJukebox.CurrentlyPlaying or localize('pbra_jukebox_default_music_title')
	-- scrolling text
	...
	if G.ALBUM_CARD.children and G.ALBUM_CARD.children.center then
		...
		local atlas = PBJukebox.Current > 0 and G.ASSET_ATLAS[current.key] or G.ASSET_ATLAS["pbra_jukebox_default"]
		local scalex = 159 / atlas.px
		local scaley = 159 / atlas.py
		local sprite = Sprite(G.ALBUM_CARD.T.x, G.ALBUM_CARD.T.y, G.ALBUM_CARD.T.w*scalex, G.ALBUM_CARD.T.h*scaley,
			atlas,
			{ x = 0, y = 0 })
		...
		G.ALBUM_CARD.pbra_jukebox_key = current and current.key
		G.ALBUM_CARD.pbra_jukebox_none = PBJukebox.Current == 0
		...
		G.ALBUM_CARD.no_tilt = true
		if G.ALBUM_CARD.pbra_jukebox_none then
	...
	if not G.ALBUM_CARD.pbra_jukebox_none then
										...
										nodes = {
											{
												...
												config = { align = "cm" },
												nodes = {
													{ n = G.UIT.T, config = { text = localize("pbra_current_track"), scale = 0.4, colour = G.C.L_BLACK } },
												}
											},
											-- Cover art (if any)
											{
								...
								nodes = {
									...
									-- Playback controls
									{
										...
										nodes = {
											...
											{
												n = G.UIT.C,
												config = { align = "cm", minh = 0.75, minw = 4, colour = G.C.GREEN, r = 0.2, emboss = 0.05, button = 'pbra_jukebox_play', button_dist = 0.1, hover = true },
												nodes = {
													create_center_aligned_text { align = "cm", text = localize('pbra_jukebox_request'), colour = G.C.UI.TEXT_LIGHT, maxw = 3.5, scale = 0.5, shadow = true }
												},
											},
										}
									},
									{
										...
										nodes = {
											...
											{
												n = G.UIT.C,
												config = { align = "cm", minh = 0.75, minw = 4, colour = G.C.RED, r = 0.2, emboss = 0.05, button = 'pbra_jukebox_stop', func = "pbra_jukebox_can_stop_playback", button_dist = 0.1, hover = true },
												nodes = {
													create_center_aligned_text { align = "cm", text = localize('pbra_jukebox_unrequest'), colour = G.C.UI.TEXT_LIGHT, maxw = 3.5, scale = 0.5, shadow = true }
												},
											},
										}
									},
									{
										...
										nodes = {
											...
											{
												n = G.UIT.C,
												config = { align = "cm", minh = 0.75, minw = 4, colour = G.C.FILTER, r = 0.2, emboss = 0.05, button = 'pbra_jukebox_shuffle', button_dist = 0.1, hover = true },
												nodes = {
													create_center_aligned_text { align = "cm", text = localize('pbra_jukebox_shuffle'), colour = G.C.UI.TEXT_LIGHT, maxw = 3.5, scale = 0.5, shadow = true }
												},
											},
										}
									}
